...
public class StudentServiceImpl implements StudentService{
	...
	public SqlSession getSqlSession() {		// 기존 getConnection과 같음
		...
		try {
			...
			// false 값은 auto commit을 해제(기본값은 true)
		} catch (IOException e) {
			e.printStackTrace();
		}
		return session;
	...
	public int insertStudent(String name) {
		SqlSession session = getSqlSession();
		int result = new StudentDAOImpl().insertStudent(session, name);
		if(result>0) {
			session.commit();
		}else {
			session.rollback();
	...
	public int insertStudent(Student st) {
		SqlSession session = getSqlSession();
		int result = new StudentDAOImpl().insertStudent(session, st);
		if(result>0) {
			session.commit();
		}else {
			session.rollback();
	...
	public int updateStudent(Student st) {
		SqlSession session = getSqlSession();	// 세션 객체 가져옴 (커넥션 객체)
		int result = new StudentDAOImpl().updateStudent(session, st);
		if(result>0) {
			session.commit();
		}else {
			session.rollback();
	...
	public Student selectOneStudent(int studentNo) {
		SqlSession session = getSqlSession();
		Student st = new StudentDAOImpl().selectOneStudent(session,studentNo);
		return st;
	}
